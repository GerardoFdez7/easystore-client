...
// Hook: initialize a Driver.js tour only once per-account.
// Behavior:
// - Wait until `profile?.email` is available (do not start with a generic key).
// - Use a per-account storage key based on the tenant email so the tour is
//   shown strictly once for that account across reloads.
...
export default function useDriverTour() {
  ...
  useEffect(() => {
    ...
    try {
      if (localStorage.getItem(storageKey)) return; // already seen for this account
    } catch (_e) {
